...
class Command(BaseCommand):
    ...
    Creates Sellers, Trades, and one SellerRawData per Trade to respect
    the unique_together(seller, trade) constraint defined in the model.

    Default behavior creates 10 sellers and 4 trades per seller (40 trades),
    thus 40 SellerRawData rows.
    """

    help = (
        "Seed Sellers, Trades, and SellerRawData with Faker. "
        "By default creates 10 sellers and 4 trades per seller (40 rows)."
    )
    ...
    def handle(self, *args, **options) -> None:
        ...
        with transaction.atomic():
            ...
            # For each seller, create the requested number of trades
            for seller in sellers:
                for t in range(trades_per_seller):
                    trade = Trade(
                        seller=seller,
                        trade_name=f"{faker.color_name()} {faker.bs().title()}",
                    )
                    trades.append(trade)
            Trade.objects.bulk_create(trades)

            # Refresh with PKs
            trades = list(Trade.objects.order_by("id"))
