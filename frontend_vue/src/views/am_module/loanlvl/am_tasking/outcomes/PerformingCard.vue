<template>
  <b-card class="w-100 h-100 border border-1 border-success rounded-2 shadow-sm" style="background: var(--ui-bg-card-primary, #FDFBF7);">
    <template #header>
      <div
      ...
      >
        <h5 class="mb-0 d-flex align-items-center">
          ...
          <UiBadge tone="success" size="lg">Performing</UiBadge>
        </h5>
...
import { useAmOutcomesStore, type PerformingTask, type PerformingTaskType } from '@/stores/outcomes'
...
const { emitTaskAdded, emitTaskDeleted } = useDataRefresh(props.hubId, async () => {
  await store.listPerformingTasks(props.hubId, true)
})

const localCollapsed = ref<boolean>(false)
const collapsed = computed(() => props.masterCollapsed || localCollapsed.value)
const tasks = computed<PerformingTask[]>(() => store.performingTasksByHub[props.hubId] ?? [])
...
const taskOptions: Array<{ value: PerformingTaskType; label: string }> = [
  { value: 'perf', label: 'Performing' },
  { value: 'rpl', label: 'Re-Performing (RPL)' },
  { value: 'note_sold', label: 'Note Sold' },
]

const tasksBusy = ref(false)
const deleteTaskConfirm = ref({ open: false, taskId: null as number | null, busy: false })

function taskLabel(v: PerformingTaskType): string {
...
function leftEdgeStyle(tp: PerformingTaskType): Record<string, string> {
  const colorMap: Record<PerformingTaskType, string> = {
    perf: 'var(--bs-success, #198754)',
    rpl: 'var(--bs-primary, #0d6efd)',
    note_sold: 'var(--bs-secondary, #6c757d)',
  }
  return {
...
function onSelectPill(tp: PerformingTaskType) {
  if (existingTypes.value.has(tp) || tasksBusy.value) return
  tasksBusy.value = true
  store.createPerformingTask(props.hubId, tp)
    .then(async (newTask) => {
      await store.listPerformingTasks(props.hubId, true)
      emitTaskAdded('performing', newTask?.id || 0)
    })
...
onMounted(async () => {
  await store.listPerformingTasks(props.hubId)
})
...
async function confirmDeleteTask() {
  ...
  try {
    ...
    await store.deletePerformingTask(props.hubId, taskId)
    await store.listPerformingTasks(props.hubId, true)
    emitTaskDeleted('performing', taskId)
    cancelDeleteTask()
  } catch (err) {
