<template>
  ...
  <b-row class="g-1 mb-1">
    ...
    <b-col lg="2" md="4" sm="6">
      ...
      <div class="card widget-flat">
        <div class="card-body py-2 px-3 position-relative">
          ...
          <h5 class="text-muted fw-normal mt-0 small" title="Interest Rate / Default Rate">Interest Rate / Default Rate</h5>
          <div class="value-row d-flex align-items-baseline">
            <h3 class="mt-1 mb-1 fs-4">{{ interestRateStr }}</h3>
            <span v-if="defaultRateStr !== '—'" class="mx-2 fs-4">/</span>
            <h3 v-if="defaultRateStr !== '—'" class="mt-1 mb-1 fs-4">{{ defaultRateStr }}</h3>
          </div>
        </div>
      </div>
    </b-col>
    ...
    <!-- Moved Flags card to the end position -->
    <b-col lg="2" md="4" sm="6">
...
const defaultRateStr = computed<string>(() => fmtPercent(props.row?.default_rate))
...
const propertyTypeStr = computed<string>(() => (props.row?.property_type ? String(props.row.property_type) : '—'))
const productTypeStr = computed<string>(() => (props.row?.product_type ? String(props.row.product_type) : '—'))
const occupancyStr = computed<string>(() => (props.row?.occupancy ? String(props.row.occupancy) : '—'))
const occupancyDisplayStr = computed<string>(() =>
  occupancyStr.value.toLowerCase() === 'unknown' ? 'Occ. Unknown' : occupancyStr.value
)
...
// Flag labels and tones (FC, BK, Mod) via centralized helpers
const fcFlagLabel = computed<string>(() => toYesNoLabel((props.row as any)?.fc_flag))
const bkFlagLabel = computed<string>(() => toYesNoLabel((props.row as any)?.bk_flag))
const modFlagLabel = computed<string>(() => toYesNoLabel((props.row as any)?.mod_flag))

const fcFlagTone = computed<BadgeToneKey>(() => getFcFlagBadgeTone((props.row as any)?.fc_flag))
const bkFlagTone = computed<BadgeToneKey>(() => getBkFlagBadgeTone((props.row as any)?.bk_flag))
const modFlagTone = computed<BadgeToneKey>(() => getModFlagBadgeTone((props.row as any)?.mod_flag))
</script>
